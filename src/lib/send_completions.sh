...
send_completions() {
  ...
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_dtools_completions_filter "--help -h drft ecs gama gcs git jira journal multi-host-log-viewer openapi prs readme s3 system-benchmark tail-logs terraform wrkflw")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'vm linux\'*\'--dist\')'
